...
import {
  EvidenceWizard,
  type EvidenceWizardBundle,
} from "../app/admin/evidence/EvidenceWizard.tsx";
...
test("loads the administrator bundle list when the server page starts empty", async () => {
  const api = async (path: string) =>
    path === "/api/admin/evidence" ? { bundles: [bundle] } : { bundle };
  const container = await render(
    <EvidenceWizard initialBundles={[]} request={api} />,
  );
  await act(async () => {
...
const spatialValue = {
  ...
  coordinateSystem: {
    width: 100,
    height: 136,
    attackDirection: "negative_y",
    normalized: true,
  },
  scene: {
    title: "픽소 판단",
    decisionTime: "00:00:05",
    userRole: "fixo",
    ballOwnerId: "F1",
    ...
    alternatives: [],
    riskyActions: [],
    expectedOutcome: "압박 회피",
    evidence: [
      {
        timeRange: "00:00:00-00:00:06",
        type: "observation",
        statement: "D1이 접근",
      },
    ],
...
test("analysis remains disabled until the operator explicitly confirms the evidence inventory", async () => {
  const container = await render(
    <EvidenceWizard initialBundles={[bundle]} initialBundleId="bundle-1" />,
  );
  ...
  const confirmation = container.querySelector<HTMLInputElement>(
    'input[name="analysis-confirmation"]',
  );
...
test("external search only starts after the explicit button", async () => {
  ...
  const api = async (path: string, init?: RequestInit) => {
    ...
    if (path.endsWith("/search"))
      return {
        search: { id: "search-1", bundleId: bundle.id, status: "queued" },
      };
  ...
  assert.deepEqual(
    ...
    [`/api/admin/evidence/${bundle.id}/search`],
  );
...
test("operator selects at most five and imports only the checked candidates", async () => {
  ...
  const container = await render(
    <EvidenceWizard
      initialBundles={[{ ...bundle, latestSearch: search }]}
      initialBundleId={bundle.id}
  ...
  const sixth = container.querySelector<HTMLInputElement>(
    'input[value="candidate-6"]',
  );
  assert.ok(sixth);
...
test("direct-only analysis remains available", async () => {
  const container = await render(
    <EvidenceWizard initialBundles={[bundle]} initialBundleId={bundle.id} />,
  );
  await act(async () => button(container, "외부 검색").click());
  await act(async () => button(container, "외부 출처 없이 분석 확인").click());
  assert.match(
    container.textContent ?? "",
    /직접 등록 1개 · 외부 보충 0개 · 영상 관찰 0개/,
  );
  const confirmation = container.querySelector<HTMLInputElement>(
    'input[name="analysis-confirmation"]',
  );
...
test("completed searches keep imported and failed candidates visible and retry only failed selections", async () => {
  const candidates = [
    {
      id: "candidate-imported",
      title: "등록된 출처",
      publisher: "협회",
      publishedAt: "2026-01-02",
    ...
    if (init?.method === "PATCH")
      return {
        search: {
          ...
          candidates: [
            candidates[0],
            { ...candidates[1], status: "selected" as const },
          ],
        },
      };
  ...
  const container = await render(
    <EvidenceWizard
      initialBundles={[{ ...bundle, latestSearch: completed }]}
      initialBundleId={bundle.id}
      request={api}
    />,
  );
  await act(async () => button(container, "외부 검색").click());
  assert.match(container.textContent ?? "", /등록된 출처/);
  assert.match(container.textContent ?? "", /등록됨/);
  assert.match(container.textContent ?? "", /실패/);
  await act(async () => button(container, "실패한 출처 다시 시도").click());
  assert.deepEqual(
    requests.map((request) => request.path),
    [
      `/api/admin/evidence/${bundle.id}/search/${completed.run.id}`,
      `/api/admin/evidence/${bundle.id}/search/${completed.run.id}/import`,
    ],
  );
  const selection = JSON.parse(String(requests[0]?.init?.body));
  assert.deepEqual(selection.selectedIds, ["candidate-failed"]);
  assert.deepEqual(selection.excludedIds, []);
});

test("analysis inventory counts reloaded external sources separately", async () => {
  const external = {
    ...bundle.sources![0]!,
    id: "external-source",
    originalFileName: "uefa.pdf",
  ...
  const container = await render(
    <EvidenceWizard
      initialBundles={[{ ...bundle, sources: [bundle.sources![0]!, external] }]}
      initialBundleId={bundle.id}
    />,
  );
  await act(async () => button(container, "분석 확인").click());
  assert.match(
    container.textContent ?? "",
    /직접 등록 1개 · 외부 보충 1개 · 영상 관찰 0개/,
  );
  assert.match(container.textContent ?? "", /외부 보충 · uefa\.pdf/);
});

test("search polling waits for a slow response and ignores it after the bundle changes", async () => {
  const queued = {
    run: {
      id: "slow-search",
      bundleId: bundle.id,
  ...
  const second = {
    ...
    id: "bundle-2",
    title: "두 번째 묶음",
    sources: [{ ...bundle.sources![0]!, bundleId: "bundle-2" }],
  };
  ...
  window.setTimeout = ((next: TimerHandler) => {
    scheduled += 1;
    callback = typeof next === "function" ? (next as () => void) : undefined;
    return scheduled;
  ...
  try {
    const container = await render(
      <EvidenceWizard
        initialBundles={[{ ...bundle, latestSearch: queued }, second]}
        initialBundleId={bundle.id}
        request={async (path) => {
          if (path.endsWith("/search/slow-search"))
    ...
    assert.equal(scheduled, 1);
    await act(async () => button(container, "두 번째 묶음").click());
    resolvePoll?.({
      search: { ...queued, run: { ...queued.run, status: "ready" as const } },
    });
    await act(async () => {
      await Promise.resolve();
    });
    await act(async () => button(container, "다이아몬드 전방 압박").click());
    await act(async () => button(container, "외부 검색").click());
    assert.match(container.textContent ?? "", /검색 대기/);
    assert.doesNotMatch(container.textContent ?? "", /후보 검토 준비/);
  } finally {
...
test("card review places cited source context beside action reasons and shows animation memory labels", async () => {
    ...
    cards: [
      {
        ...
        updatedAt: 1,
        content: {
          situation: "전방 압박을 받는 픽소",
          conditions: ["중앙 패스길 차단"],
          defenseType: "front_press",
          cues: ["수비 발 방향"],
          preferred: [
            {
              action: "dribble",
              reason: "압박수비 바깥 어깨를 넘어 전진",
              citationIds: ["chunk-2"],
            },
          ],
          alternatives: [],
        ...
        citations: [
          {
            ...
            videoClipId: null,
            locationLabel: "문서 3쪽",
            excerpt: "수비가 중앙을 닫으면 바깥쪽으로 운반한다.",
          },
        ],
      },
    ],
  ...
  const container = await render(
    <EvidenceWizard
      initialBundles={[reviewBundle]}
      initialBundleId="bundle-1"
    />,
  );
  ...
  for (const label of ["관찰", "선택", "결과", "기억"])
...
test("multi-file upload reloads and displays every stored JSON source", async () => {
  ...
  const api = async (path: string, init?: RequestInit) => {
    if (path.endsWith("/files") && init?.body instanceof FormData) {
      ...
      const source = {
        ...
        mediaType: "text/plain",
        byteSize: file.size,
        contentHash: `hash-${stored.length + 1}`,
        extractionStatus: "completed",
        extractionError: null,
      };
      stored.push(source);
    ...
    if (path === `/api/admin/evidence/${bundle.id}`)
      return { bundle: { ...bundle, sources: stored }, latestJob: null };
    throw new Error(`unexpected ${path}`);
  };
  const container = await render(
    <EvidenceWizard
  ...
  Object.defineProperty(input, "files", {
    configurable: true,
    value: [
      new File([JSON.stringify(spatialValue)], "one.json", {
      ...
      new File(
        [
          JSON.stringify({
            ...spatialValue,
            source: { ...spatialValue.source, title: "두 번째" },
          }),
        ],
        "two.json",
        { type: "application/json" },
...
test("unsupported combined JSON reports its file name before uploading anything", async () => {
  ...
  Object.defineProperty(input, "files", {
    ...
    value: [
      new File(
        [JSON.stringify({ datasetId: "draft", sourceOverview: {}, cards: [] })],
        "combined-draft.json",
        { type: "application/json" },
      ),
    ],
