...
type Source = { id: string; bundleId: string; originalFileName: string; mediaType: string; byteSize: number; contentHash: string; extractionStatus: string; extractionError: string | null; origin?: "uploaded" | "external_web" };
...
type Citation = { chunkId: string; sourceId: string | null; videoClipId: string | null; locationLabel: string; excerpt: string; origin?: "uploaded" | "external_web" | "video_observation"; canonicalUrl?: string; publisher?: string; publishedAt?: string; retrievedAt?: number };
...
type SearchStatus = "queued" | "searching" | "ready" | "importing" | "completed" | "failed";
type SearchCandidate = { id: string; title: string; publisher: string; publishedAt: string; canonicalUrl: string; documentType: "web_page" | "pdf"; quote: string; relevance: string; trustTier: 1 | 2 | 3; rank: number; status: "candidate" | "selected" | "excluded" | "importing" | "imported" | "failed"; failureReason: string | null };
type SafeSearchRunDetail = { run: { id: string; bundleId: string; bundleVersion: number; status: SearchStatus; errorMessage: string | null; isStale: boolean; startedAt: number | null; completedAt: number | null; createdAt: number; updatedAt: number }; candidates: SearchCandidate[] };
...
export type EvidenceWizardBundle = {
  ...
  sources?: Source[]; videoClips?: Clip[]; latestJob?: Job; latestSearch?: SafeSearchRunDetail; cards?: Card[];
};
...
function searchStatusLabel(status: SearchStatus): string { return ({ queued: "검색 대기", searching: "외부 출처 검색 중", ready: "후보 검토 준비", importing: "선택 출처 가져오는 중", completed: "외부 출처 등록 완료", failed: "외부 검색 중단" })[status]; }
function candidateStatusLabel(status: SearchCandidate["status"]): string { return ({ candidate: "후보", selected: "선택됨", excluded: "제외됨", importing: "가져오는 중", imported: "등록됨", failed: "실패" })[status]; }
function localSearch(run: { id: string; bundleId: string; status: SearchStatus }, version: number): SafeSearchRunDetail {
  return { run: { ...run, bundleVersion: version, errorMessage: null, isStale: false, startedAt: null, completedAt: null, createdAt: Date.now(), updatedAt: Date.now() }, candidates: [] };
}
...
export function EvidenceWizard({ initialBundles, initialBundleId, request = defaultRequest }: Props) {
  ...
  const [candidateSelection, setCandidateSelection] = useState({ runId: "", selected: [] as string[], excluded: [] as string[] });
  ...
  const savedSelection = { selected: search?.candidates.filter((candidate) => candidate.status === "selected").map((candidate) => candidate.id) ?? [], excluded: search?.candidates.filter((candidate) => candidate.status === "excluded").map((candidate) => candidate.id) ?? [] };
  const selectedCandidateIds = candidateSelection.runId === search?.run.id ? candidateSelection.selected : savedSelection.selected;
  const excludedCandidateIds = candidateSelection.runId === search?.run.id ? candidateSelection.excluded : savedSelection.excluded;
  ...
  const loadBundle = async (id: string, includeLatestSearch = true) => {
    ...
    let next: EvidenceWizardBundle = { ...result.bundle, latestJob: result.latestJob ?? undefined };
    if (includeLatestSearch) {
      const latestSearch = await request(`/api/admin/evidence/${id}/search`) as { search: SafeSearchRunDetail | null };
  ...
  useEffect(() => {
    ...
    if (!bundle || !run || !["queued", "searching", "importing"].includes(run.status)) return;
    let active = true;
    const runId = run.id;
    const poll = async () => {
      try {
        const result = await request(`/api/admin/evidence/${bundle.id}/search/${runId}`) as { search: SafeSearchRunDetail };
        if (!active || result.search.run.id !== runId) return;
        updateBundle({ ...bundle, latestSearch: result.search });
        if (result.search.run.status === "completed") void loadBundle(bundle.id).catch(fail);
      } catch (value) { if (active) fail(value); }
    };
    const timer = window.setInterval(() => void poll(), 2000);
    return () => { active = false; window.clearInterval(timer); };
  ...
  const directCount = bundle?.sources?.filter((source) => source.origin !== "external_web").length ?? 0;
  const externalCount = bundle?.sources?.filter((source) => source.origin === "external_web").length ?? 0;
  ...
  const startSearch = async () => {
    ...
    setBusy(true); setError("");
    try {
      const result = await request(`/api/admin/evidence/${bundle.id}/search`, { method: "POST" }) as { search: { id: string; bundleId: string; status: SearchStatus } };
      updateBundle({ ...bundle, latestSearch: localSearch(result.search, bundle.version) });
      setCandidateSelection({ runId: result.search.id, selected: [], excluded: [] });
      setMessage("외부 출처 검색을 시작했습니다. 검색이 끝나면 후보를 직접 고릅니다.");
    } catch (value) { fail(value); } finally { setBusy(false); }
  };

  const saveSelection = async (): Promise<SafeSearchRunDetail | null> => {
    if (!bundle || !search) return null;
    const result = await request(`/api/admin/evidence/${bundle.id}/search/${search.run.id}`, {
      method: "PATCH", headers: { "content-type": "application/json" },
      body: JSON.stringify({ expectedBundleVersion: bundle.version, selectedIds: selectedCandidateIds, excludedIds: excludedCandidateIds }),
    }) as { search: SafeSearchRunDetail };
    updateBundle({ ...bundle, latestSearch: result.search });
    setMessage(`선택 ${selectedCandidateIds.length}개를 저장했습니다.`);
    return result.search;
  };

  const saveSearchSelection = async () => {
    setBusy(true); setError("");
    try { await saveSelection(); } catch (value) { fail(value); } finally { setBusy(false); }
  };
  ...
  const importSelection = async () => {
    ...
    try {
      ...
      if (!saved) return;
      const result = await request(`/api/admin/evidence/${bundle.id}/search/${saved.run.id}/import`, { method: "POST" }) as { search: { id: string; bundleId: string; status: SearchStatus } };
      updateBundle({ ...bundle, latestSearch: localSearch(result.search, bundle.version) });
  ...
  const toggleCandidate = (candidate: SearchCandidate) => {
    ...
    if (!selected && selectedCandidateIds.length >= 5) { setError("외부 출처는 최대 5개까지 선택할 수 있습니다."); setMessage(""); return; }
    setCandidateSelection({ runId: search?.run.id ?? "", selected: selected ? selectedCandidateIds.filter((id) => id !== candidate.id) : [...selectedCandidateIds, candidate.id], excluded: excludedCandidateIds.filter((id) => id !== candidate.id) });
    setError("");
  };

  const excludeCandidate = (candidate: SearchCandidate) => {
    if (["importing", "imported"].includes(candidate.status)) return;
    setCandidateSelection({ runId: search?.run.id ?? "", selected: selectedCandidateIds.filter((id) => id !== candidate.id), excluded: excludedCandidateIds.includes(candidate.id) ? excludedCandidateIds.filter((id) => id !== candidate.id) : [...excludedCandidateIds, candidate.id] });
  };
  ...
  return <main className="evidence-admin-shell">
    ...
    <div className="evidence-layout">
      ...
      <section className="evidence-workspace">
        ...
        {step === "sources" && <div className="evidence-panel"><span className="panel-number">02 · 직접 근거</span><h2>원문과 관찰을 함께 등록하세요</h2>{!bundle ? <p>먼저 자료 정보에서 근거 묶음을 만들어 주세요.</p> : <><div className="upload-drop"><input id="evidence-files" type="file" multiple accept=".pdf,.txt,.md,.markdown,.json,application/pdf,application/json,text/plain,text/markdown" onChange={upload} disabled={busy}/><label htmlFor="evidence-files"><strong>PDF · TXT · Markdown · JSON 업로드</strong><span>파일당 최대 20MB · JSON은 공간 전술 표준 형식만 허용</span></label></div><form className="clip-form json-form" onSubmit={addSpatialJson}><h3>공간 전술 JSON 붙여넣기</h3><p>선수 좌표, 수비 형태, 판단 단서와 행동 경로를 포함한 표준 JSON을 검사한 뒤 저장합니다.</p><label>JSON 원문<textarea name="spatial-json" required value={spatialJson} onChange={(event) => setSpatialJson(event.target.value)} placeholder={'{\n  "source": { ... },\n  "coordinateSystem": { ... },\n  "scene": { ... }\n}'}/></label><button disabled={busy || !spatialJson.trim()}>JSON 근거 저장</button></form><div className="source-list">{(bundle.sources ?? []).map((source) => <article key={source.id}><div><strong>{source.originalFileName}</strong><small>{Math.ceil(source.byteSize / 1024)}KB · {source.extractionStatus === "completed" ? "텍스트 준비됨" : "원문 보관 · OCR 필요"}</small></div><a href={`/api/admin/evidence/${bundle.id}/files/${source.id}`}>원문</a><button onClick={() => void inspectDelete(source.id)}>삭제</button></article>)}</div><form className="clip-form" onSubmit={addClip}><h3>영상 구간 직접 기록</h3><label>HTTPS 영상 URL<input required type="url" value={clip.url} onChange={(event) => setClip({ ...clip, url: event.target.value })}/></label><div><label>시작<input required value={clip.start} onChange={(event) => setClip({ ...clip, start: event.target.value })}/></label><label>종료<input required value={clip.end} onChange={(event) => setClip({ ...clip, end: event.target.value })}/></label></div><label>이 구간에서 확인한 사실<textarea required value={clip.observation} onChange={(event) => setClip({ ...clip, observation: event.target.value })}/></label><button disabled={busy}>영상 구간 추가</button></form><button className="evidence-primary" disabled={!inventoryReady} onClick={() => setStep("search")}>외부 출처 검토하기</button></>}</div>}
        {step === "search" && <div className="evidence-panel search-panel"><span className="panel-number">03 · 외부 검색</span><h2>외부 출처는 직접 검토한 뒤 보충하세요</h2>{!bundle ? <p>먼저 자료 정보에서 근거 묶음을 만들어 주세요.</p> : <><p className="search-guidance">검색은 버튼을 누를 때만 시작합니다. 후보와 원문을 확인하고 최대 5개만 선택하세요.</p>{search ? <div className={`job-status search-status ${search.run.status}`} aria-live="polite"><strong>{searchStatusLabel(search.run.status)}</strong><span>{search.run.isStale ? "자료 변경으로 이전 검색입니다." : "검색 결과와 가져오기 상태를 여기에서 확인합니다."}</span>{search.run.errorMessage && <p role="alert">{search.run.errorMessage}</p>}{search.run.status === "failed" && <button onClick={() => void startSearch()} disabled={busy}>다시 시도</button>}</div> : <button className="evidence-primary" onClick={() => void startSearch()} disabled={busy}>외부 출처 찾기</button>}{search?.run.status === "ready" && <><div className="selection-summary" aria-live="polite"><strong>선택 {selectedCandidateIds.length}/5</strong><span>선택한 출처만 저장하거나 가져올 수 있습니다.</span></div><div className="candidate-list">{search.candidates.map((candidate) => { const selected = selectedCandidateIds.includes(candidate.id); const excluded = excludedCandidateIds.includes(candidate.id); const expanded = expandedQuotes.includes(candidate.id); return <article className="candidate-card" key={candidate.id}><label className="candidate-select"><input type="checkbox" value={candidate.id} checked={selected} disabled={busy || candidate.status === "importing" || candidate.status === "imported"} onChange={() => toggleCandidate(candidate)}/><span><strong>{candidate.title}</strong><small>{candidateStatusLabel(selected ? "selected" : excluded ? "excluded" : candidate.status)}</small></span></label><p>{candidate.publisher} · {candidate.publishedAt} · {candidate.documentType === "pdf" ? "PDF" : "웹"} · 서버 신뢰 등급 {candidate.trustTier}</p><blockquote className={expanded ? "is-expanded" : ""}>{candidate.quote}</blockquote>{candidate.quote.length > 160 && <button className="quote-toggle" onClick={() => setExpandedQuotes((current) => current.includes(candidate.id) ? current.filter((id) => id !== candidate.id) : [...current, candidate.id])}>{expanded ? "인용 접기" : "인용 펼치기"}</button>}<p>관련성: {candidate.relevance}</p><a href={candidate.canonicalUrl} target="_blank" rel="noreferrer noopener">새 탭에서 원문 보기</a><button className="candidate-exclude" onClick={() => excludeCandidate(candidate)} disabled={busy || candidate.status === "importing" || candidate.status === "imported"}>{excluded ? "제외 취소" : "제외"}</button>{candidate.failureReason && <p role="alert">{candidate.failureReason}</p>}</article>; })}</div><div className="search-actions"><button onClick={() => void saveSearchSelection()} disabled={busy}>선택 저장</button><button className="evidence-primary" onClick={() => void importSelection()} disabled={busy || selectedCandidateIds.length === 0}>선택 출처 가져오기</button></div></>}{search?.run.status === "completed" && <p aria-live="polite">외부 보충 출처 등록이 끝났습니다. 아래에서 분석 범위를 확인하세요.</p>}<button className="direct-only" onClick={() => setStep("analysis")}>외부 출처 없이 분석 확인</button></>}</div>}
        {step === "analysis" && <div className="evidence-panel"><span className="panel-number">04 · 분석 확인</span><h2>분석 범위를 마지막으로 확인하세요</h2>{!bundle ? <p>선택한 근거 묶음이 없습니다.</p> : <><div className="inventory"><strong>{bundle.title}</strong><span>직접 등록 {directCount}개 · 외부 보충 {externalCount}개 · 영상 관찰 {bundle.videoClips?.length ?? 0}개</span>{(bundle.sources ?? []).map((source) => <p key={source.id}>{source.origin === "external_web" ? "외부 보충" : "직접 등록"} · {source.originalFileName}</p>)}{(bundle.videoClips ?? []).map((item, index) => <p key={item.id ?? index}>영상 관찰 · {item.url} · {item.observation}</p>)}</div><label className="confirm-row"><input aria-label="아래에서 확인한 직접·외부 근거만 분석한다는 점을 확인했습니다" name="analysis-confirmation" type="checkbox" checked={confirmation} onChange={(event) => setConfirmation(event.target.checked)}/><span><strong>아래에서 확인한 직접·외부 근거만 분석한다는 점을 확인했습니다</strong><small>인터넷 지식이나 출처 없는 전술 설명은 정답으로 사용하지 않습니다.</small></span></label><button className="evidence-primary" disabled={!inventoryReady || !confirmation || busy || job?.status === "running" || job?.status === "queued"} onClick={() => void startAnalysis()}>분석 시작</button>{job && <div className={`job-status ${job.status}`} aria-live="polite"><strong>{job.status === "review_ready" ? "카드 검수 준비 완료" : job.status === "failed" ? "분석 중단" : "근거 분석 중"}</strong><span>단계: {job.stage} · 시도 {job.attemptCount}회</span>{job.isStale && <p role="alert">자료가 변경되어 이 분석은 오래된 버전입니다.</p>}{job.status === "failed" && <button onClick={() => void retryJob()}>분석 다시 시도</button>}{job.status === "review_ready" && <button onClick={() => setStep("review")}>카드 검수로 이동</button>}</div>}</>}</div>}
        {step === "review" && <div className="evidence-panel review-panel"><span className="panel-number">05 · 카드 검수</span><h2>근거와 설명을 나란히 확인하세요</h2>{!bundle?.cards?.length ? <p className="empty-review">분석이 완료되면 검수할 카드가 여기에 표시됩니다.</p> : bundle.cards.map((card) => <article className="review-card" key={card.id}><header><div><span>{defenseLabels[card.content.defenseType] ?? card.content.defenseType}</span><h3>{card.content.situation}</h3></div><em>{card.content.confidence === "high" ? "높은 확신" : card.content.confidence === "medium" ? "중간 확신" : "낮은 확신"}</em></header><div className="review-columns"><section aria-label="출처 근거"><h4>코치 원문 근거</h4>{card.citations.map((citation) => <div key={citation.chunkId} ref={(node) => { if (node) citationRefs.current.set(citation.chunkId, node); }} className="citation-excerpt" id={`citation-${citation.chunkId}`}><strong>근거 {citationLabel(citation.chunkId)} · {citation.locationLabel}</strong><span className="origin-badge">{citation.origin === "external_web" ? "외부 보충" : citation.origin === "video_observation" ? "영상 관찰" : "직접 등록"}</span><span>{citation.excerpt}</span>{citation.origin === "external_web" && citation.publisher && citation.publishedAt && citation.retrievedAt && citation.canonicalUrl && <small>{citation.publisher} · 게시 {citation.publishedAt} · 수집 {new Date(citation.retrievedAt).toLocaleDateString("ko-KR")}</small>}{citation.origin === "external_web" && citation.canonicalUrl && <a href={citation.canonicalUrl} target="_blank" rel="noreferrer noopener">새 탭에서 원문 보기</a>}</div>)}</section><section aria-label="전술 카드 설명"><h4>선수에게 보여줄 설명</h4><dl><dt>조건</dt><dd>{card.content.conditions.join(" · ") || "없음"}</dd><dt>관찰 단서</dt><dd>{card.content.cues.join(" · ") || "없음"}</dd></dl>{(["preferred", "alternatives", "risky"] as const).map((group) => card.content[group].map((item, index) => <div className={`action-reason ${group}`} key={`${group}-${index}`}><strong>{actionLabels[item.action]}{item.tacticalIntent ? ` · ${intentLabels[item.tacticalIntent] ?? item.tacticalIntent}` : ""} {group === "preferred" ? "권장 이유" : group === "alternatives" ? "대안 이유" : "주의 이유"}</strong>{item.actorId && <small>{item.actorId}{item.targetId ? ` → ${item.targetId}` : ""}{item.trigger ? ` · 조건: ${item.trigger}` : ""}</small>}<p>{item.reason}</p><div>{item.citationIds.map((id) => <button key={id} onClick={() => citationRefs.current.get(id)?.scrollIntoView({ behavior: "smooth", block: "center" })}>근거 {citationLabel(id)}</button>)}</div></div>))}</section></div><div className="animation-memory"><strong>직관적 애니메이션 설명</strong><ol><li><span>관찰</span>수비 위치와 열린 공간을 본다</li><li><span>선택</span>신체 행동과 전술 의도를 함께 확인한다</li><li><span>결과</span>경로·대상·발동 조건이 어떻게 연결되는지 본다</li><li><span>기억</span>경기에서 확인할 한 문장으로 남긴다</li></ol></div>{card.content.conflicts.length > 0 && <p role="alert" className="conflict">미해결 충돌: {card.content.conflicts.join(" · ")}</p>}<footer><button onClick={() => void review(card, "rejected")}>반려</button><button onClick={() => void review(card, "held")}>보류</button><button className="approve" disabled={!cardReady(card) || busy} onClick={() => void review(card, "owner_reviewed")}>본인 검수 완료</button></footer></article>)}</div>}
      </section>
    </div>
